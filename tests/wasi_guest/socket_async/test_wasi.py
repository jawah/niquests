...
async def test_capabilities_and_adapter_selection() -> None:
    async with niquests.AsyncSession() as session:
        ...
        assert isinstance(session.get_adapter("https://httpbingo.org"), AsyncHTTPAdapter)
...
async def test_real_http2_trailers() -> None:
    async with niquests.AsyncSession() as session:
        response = await session.get("https://httpbingo.org/trailers?trailer1=value1&trailer2=value2")
